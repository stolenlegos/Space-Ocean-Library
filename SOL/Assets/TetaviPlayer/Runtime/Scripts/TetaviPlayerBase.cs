...
public class TetaviPlayerBase : MonoBehaviour
{
    ...
    protected bool userPaused = false;
    ...
    protected Mesh CreateMesh(IntPtr frame)
    {
        ...
        frame_copy_face_indices(frame, pointer2); // --- if it is not a key frame we don't need to use it
                                                  //UV
    ...
    bool inStress = false;
    protected bool UpdateFrame(int frameIdx)
    {
...
#endif
#endif
        if (clipFile!="" && clipFile != "..." && !clipFile.Contains("/"))
        {
    ...
    protected IEnumerator Start()
    {
        ...
        {
            Debug.LogError($"Clip file by path {pathToFile} does not exist");
        }
        yield return null;
    }
